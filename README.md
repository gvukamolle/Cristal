...
**macOS:**
- Откройте Finder → перейдите в нужную папку → ПКМ на папке → "Новый терминал в папке"
- Или в терминале: `cd /путь/к/папке`

**Windows:**
- Откройте папку в Проводнике → зажмите Shift + ПКМ → "Открыть окно PowerShell здесь"

**VS Code:**
- Откройте папку → меню Terminal → New Terminal (`Ctrl + ~`)
